...
const surveyURL = 'https://forms.gle/Ca4wkZQmtibyVt1D9';
...
const aboutImg = '../images/Talkative-logo.png';
...
let recStartTime;
let recElapsedTime;
let TalkativeTheme = 'dark'; // dark - grey ...
let TalkativeBtnsBar = 'vertical'; // vertical - horizontal
...
function getHtmlElementsById() {
    ...
    themeSelect = getId('TalkativeTheme');
    videoObjFitSelect = getId('videoObjFitSelect');
    btnsBarSelect = getId('TalkativeBtnsBar');
...
function welcomeUser() {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            let message = {
                email: '',
                subject: 'Please join our Talkative Video Chat Meeting',
            ...
            switch (event.channel.label) {
                case 'Talkative_chat_channel':
                    try {
                        ...
                        console.error('Talkative_chat_channel', err);
                    }
                    break;
                case 'Talkative_file_sharing_channel':
                    try {
                        let dataFile = msg.data;
                        handleDataChannelFileSharing(dataFile);
                    } catch (err) {
                        console.error('Talkative_file_sharing_channel', err);
                    }
                    break;
            }
        };
...
/**
 * Set Talkative theme | dark | grey | ...
 * @param {string} theme type
 */
function setTheme(theme) {
    if (!theme) return;

    TalkativeTheme = theme;
    switch (TalkativeTheme) {
        case 'dark':
            // dark theme
    ...
    setButtonsBarPosition(TalkativeBtnsBar);
}
...
function setButtonsBarPosition(position) {
    ...
    TalkativeBtnsBar = position;
    switch (TalkativeBtnsBar) {
        case 'vertical':
...
function createChatDataChannel(peer_id) {
    chatDataChannels[peer_id] = peerConnections[peer_id].createDataChannel('Talkative_chat_channel');
...
function createFileSharingDataChannel(peer_id) {
    fileDataChannels[peer_id] = peerConnections[peer_id].createDataChannel('Talkative_file_sharing_channel');
...
function selectFileToShare() {
    ...
    Swal.fire({
        allowOutsideClick: false,
        background: swalBackground,
        imageAlt: 'Talkative-file-sharing',
    ...
    if (isImageURL(incomingFileInfo.fileName)) {
        ...
        reader.onload = (e) => {
            Swal.fire({
                ...
                imageAlt: 'Talkative-file-img-download',
                showDenyButton: true,
        ...
        Swal.fire({
            allowOutsideClick: false,
            background: swalBackground,
            imageAlt: 'Talkative-file-download',
            imageUrl: fileSharingImg,
            position: 'center',
            title: 'Received file',
            text: incomingFileInfo.fileName + ' size ' + bytesToSize(incomingFileInfo.fileSize),
