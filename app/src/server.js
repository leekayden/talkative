...
/**
 * Talkative P2P - Server component
 *
...
const api_key_secret = process.env.API_KEY_SECRET || 'Talkative_default_secret';
...
app.get('/join/', (req, res) => {
    if (Object.keys(req.query).length > 0) {
        ...
        /* 
            http://localhost:3000/join?room=test&name=Talkative&audio=1&video=1&screen=1&notify=1
            https://Talkative.up.railway.app/join?room=test&name=Talkative&audio=1&video=1&screen=1&notify=1
            https://Talkative.herokuapp.com/join?room=test&name=Talkative&audio=1&video=1&screen=1&notify=1
        */
...
app.post([apiBasePath + '/meeting'], (req, res) => {
    ...
    if (authorization != api_key_secret) {
        log.debug('Talkative get meeting - Unauthorized', {
            header: req.headers,
            body: req.body,
        });
    ...
    log.debug('Talkative get meeting - Authorized', {
        header: req.headers,
        body: req.body,
        meeting: meetingURL,
    });
